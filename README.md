# Flluid Studio

一个流体着色卡片交互演示：中心一张 256×256 的 WebGL 流体卡片，左右两侧可分别切换**运动模式**（Still / Wave / Storm）与**色彩主题**（Mist / Ocean / Abyss），卡片与选项之间用流动虚线连接。界面采用 Pearl Flowglass 共享 token；GPU 不可用时会降级为可交互的 CSS 流体表面。

## 特性

- **WebGL 流体着色** — Three.js 实现的实时流体形变着色，卡片表面随时间流动
- **物理形态解耦控制** — 扭曲度 `warp`、折痕深度 `fold`、振幅 `amp`、流动速度 `speed` 独立可调
- **三种运动模式**
  - `Still` — 静止（低扭曲、低振幅）
  - `Wave` — 波浪（默认）
  - `Storm` — 风暴（高扭曲、高折叠）
- **三种色彩主题**（从视频逐帧提取的色彩矩阵）
  - `Mist` — 粉雾浅蓝，低饱和通透
  - `Ocean` — 钢蓝渐变色谱
  - `Abyss` — 黑曜石墨蓝
- **选项联动** — 卡片与左右选项之间有流动虚线 + 端点圆点连接
- **响应式** — 移动端自动切换为纵向布局，隐藏连接线
- **无障碍** — 键盘可操作选项（`Tab` + `Enter`），支持 `prefers-reduced-motion`

## 运行

无需构建，直接双击打开 `index.html`，或本地起一个静态服务器：

```bash
npx serve .
```

依赖（CDN）：

- Three.js `r128`
- GSAP `3.12.5`

## 交互

| 操作 | 效果 |
|---|---|
| 鼠标悬停运动 / 色彩选项 | 临时预览效果，不更改状态 |
| 点击选项 | 提交运动或色彩选择 |
| `Tab` 聚焦 + `Enter` / `Space` | 键盘提交选择 |

## 技术说明

- 流体效果由 Three.js fragment shader 驱动，卡片圆角 `46px` 内裁切
- 卡片外围有柔光光晕（`glow`），表面叠加内高光/内阴影质感
- 虚线连接线使用 SVG `stroke-dasharray` 动画，方向随选项激活状态流动
